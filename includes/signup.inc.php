...
if (isset($_POST['submit'])) { /** a for loop to check if the submit button exists */
    ...
    if (empty($user_name) || empty($first_name) || empty($last_name) || empty($email) || empty($confirm_email) || empty($password)) { /** check if the first field is empty or the next field and repeat */
        header("Location: ../register.php?register=empty"); /** If one or more fields are empty then send the user back to the registration page and include error message in the url */
        ...
        if (!preg_match("/^[a-zA-Z]*$/", $user_name) || !preg_match("/^[a-zA-Z]*$/", $first_name) || !preg_match("/^[a-zA-Z]*$/", $last_name)) { /** Use the php function (preg_match) to check if not the specified characters have been entered within the input fields */
            header("Location: ../register.php?register=invalid"); /** If one or more fields contain incorrect characters then send the user back to the registration page and include error message in the url */
            exit(); /** Stop this script from running */
        } else {
            /** Check for email validation */
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                header("Location: ../register.php?register=invalidemail"); /** If one or more fields contain incorrect characters then send the user back to the registration page and include error message in the url */
                exit(); /** Stop this script from running */
            } else {
                if (!filter_var($confirm_email, FILTER_VALIDATE_EMAIL)) {
                    ...
                    if ($resultCheck > 0) { /** Check if the username is greater than 0 in the database, if it is then show an error message */
                        header("Location: ../register.php?register=usernametaken"); /** If the username already exists then send the user back to the registration page and include error message in the url */
                        exit(); /** Stop this script from running */
                    } else {
                        /**
                         *
